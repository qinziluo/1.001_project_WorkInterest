...
var cleardata = function (data) {//get the useful data
    ...
    var newresults = [];
    var everyjob = {};
    info = ['jobtitle','city','company', 'url' ];//select the useful information
...
// 'Austin', 'Boston', 'Chicago', 'Dallas', 'Houston', 'LosAngeles', 'NewYork', "Philadelphia",
//     'SanDiego', 'Sanfrancisco', 'Seattle', 'Washington'

for (var i = 0; i < city.length; i++) {
    var data = require('./' + city[i] + '.json');
    console.log('hi');
    var alldata = cleardata(data);
    fs.writeFile(city[i] + 'alldata.json', JSON.stringify(alldata));
    ...
    fs.writeFile(city[i]+'companydata.json', 'var '+city[i]+'companydata=');
    fs.writeFile(city[i] + 'companydata.json', JSON.stringify(sortedcompanydata));
    ...
    fs.writeFile(city[i]+'jobdata.json', 'var '+city[i]+'jobdata=');
    fs.writeFile(city[i] + 'jobdata.json', JSON.stringify(sortedjobdata));
